...
vi.mock("../lib/plans/repository", () => ({
  findPlanEngagementId: vi.fn(async (_actor, id) => id === "p2" ? "e2" : undefined),
...
it.each([{ engagement: "e2", plan: "p2" }, { plan: "p2" }])("keeps an older engagement plan outside the global list: %j", async (query) => {
  vi.mocked(listPlans).mockImplementation(async (_actor, engagementId) => engagementId === "e2" ? [{ planId: "p2", engagementId: "e2", title: "Older plan", versionNumber: 1, updatedAt: "2026-01-01" }] : []);
