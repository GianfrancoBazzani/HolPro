...
vi.mock("../lib/pro/ownership", () => ({
  ownsEngagement: vi.fn(async () => true),
}));
vi.mock("../lib/calendar/draft-repository", () => ({
  applyCalendarDraft: vi.fn(),
  discardCalendarDraft: vi.fn(),
}));
import { ownsEngagement } from "../lib/pro/ownership";
import * as repository from "../lib/calendar/draft-repository";
...
const form = () => {
  ...
  f.set("engagementId", id);
  f.set("draftId", draftId);
  return f;
};
beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(ownsEngagement).mockResolvedValue(true);
});
for (const [action, write] of [
  [approveCalendarDraft, repository.applyCalendarDraft],
  [discardCalendarDraft, repository.discardCalendarDraft],
] as const) {
  ...
  it(`${action.name} denies another coach`, async () => {
    vi.mocked(ownsEngagement).mockResolvedValue(false);
