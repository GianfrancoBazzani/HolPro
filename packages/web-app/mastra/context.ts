...
export async function buildInstructions(
  ...
  return [
    ...
    ...(context.role === "coachee"
      ...
      : []),
    ...(context.role === "coach" ? ["For every calendar change, first call getClientPlan and reuse the real item, checkpoint and period ids from its result. Use the engagementId supplied by the current client page context when present. Submit every calendar change through proposeCalendarChanges. This is a proposal: the coach must approve it in the dashboard. Never report the calendar as changed before the coach approves it. Do not place child operations after deleting their parent item."] : []),
    planInstructions,
